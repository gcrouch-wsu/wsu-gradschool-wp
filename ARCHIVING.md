...
5. Leave credentials blank for the first run. If a planned download from the WordPress host fails with 401 or 403, retry with a dedicated WordPress username and Application Password. Credentials do not discover file URLs or resolve missing records; they remain in process memory and are not saved.
