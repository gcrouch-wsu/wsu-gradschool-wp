...
Live WordPress states (found, in Trash, missing) belong to the analyzed export, not to the site: analyzing a replacement export resets every record to **Not checked**, and the "Live WordPress" filter — including **In trash** — stays empty until records are checked. A **Next step** callout directly under the group header says so and contains the **Check live WordPress** button; it checks every record in the selected group (or the work queue) in batches of 100 with a progress bar, then turns green with counts of records in Trash, missing, or in error, and offers a re-check. Opening a record in the review sheet still checks that single record.

Choosing an export shows a waiting state on the drop zone from the moment the file picker opens until the browser hands the file over (large exports, and files kept in OneDrive or on a network drive, can take a minute to attach); the zone turns green with the file name and size when it is ready and the button is enabled. During upload the overlay shows the upload step with bytes sent, the single analysis step with an elapsed clock, and then the dashboard opening. If the analysis response is lost (for example a gateway timeout on a very large export), the page polls `/api/upload-status` and adopts the finished report instead of re-analyzing; completing the same upload twice never re-analyzes.
